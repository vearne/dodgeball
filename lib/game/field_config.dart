...
class FieldConfig {
  ...
  static Rect getRedTeamArea(Vector2 gameSize) {
    ...
    // 计算可用宽度（减去左右边距和中间间隔）
    final availableWidth = playableArea.width - playAreaMargin * 2 - centerGap;
    final teamWidth = availableWidth / 2;
  ...
  static Rect getBluTeamArea(Vector2 gameSize) {
    ...
    return Rect.fromLTWH(
      playableArea.left + playAreaMargin + teamWidth + centerGap,
      playableArea.top + playAreaMargin,
      teamWidth,
      playableArea.height - playAreaMargin * 2,
    );
